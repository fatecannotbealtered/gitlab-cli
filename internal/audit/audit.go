...
// Dir returns the audit log directory ~/.gitlab-cli/audit/
func Dir() string {
	if testDir != "" {
		return testDir
	}
...
// It is a no-op if GITLAB_NO_AUDIT=1. It performs lazy cleanup of old files.
func Log(cmdPath string, args []string, exitCode int, durationMs int64) {
	if os.Getenv("GITLAB_NO_AUDIT") == "1" {
		return
	}
...
// Defaults to 3. Set GITLAB_AUDIT_RETENTION_MONTHS=0 to disable cleanup.
func retentionMonths() int {
	s := os.Getenv("GITLAB_AUDIT_RETENTION_MONTHS")
	if s == "" {
		return 3
	}
...
// Handles: --token value, --token=value, -t value, -t=value (case-insensitive).

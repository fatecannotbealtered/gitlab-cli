...
<p align="center">
  <strong>Agent-native GitLab CLI &middot; JSON-first &middot; dry-run guarded</strong>
</p>
...
| CI/CD | `pipeline ...`, `job ...`, `variable ...` | Inspect, wait, retry, cancel, play manual jobs, download artifacts, and manage CI variables. `job log` supports token-efficient `--tail`/`--grep`/`--max-bytes`. |

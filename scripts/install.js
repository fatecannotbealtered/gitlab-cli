...
function install() {
  if (process.env.GITLAB_CLI_SKIP_INSTALL === '1' || process.env.SKIP_INSTALL === '1') {
    console.log('GITLAB_CLI_SKIP_INSTALL set, skipping binary download.');
    process.exit(0);
  }

  if (fs.existsSync(dest) && !process.env.GITLAB_CLI_FORCE_INSTALL) {
    console.log(`${NAME} already installed at ${dest}, skipping download. Set GITLAB_CLI_FORCE_INSTALL=1 to reinstall.`);
    process.exit(0);
  }

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gitlab-cli-"));

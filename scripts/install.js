...
// postinstall downloader for the prebuilt gitlab-cli binary.
// Language-agnostic: the release archive may hold a Go OR Python (e.g. PyInstaller) binary;
// this script only cares that bin/gitlab-cli[.exe] ends up in place.
...
const REPO = "fatecannotbealtered/gitlab-cli";
const NAME = "gitlab-cli";

// Env overrides: skip entirely (offline / source build / CI), or force a re-download.
const SKIP = process.env["GITLAB_CLI_SKIP_INSTALL"] || process.env.SKIP_INSTALL;
const FORCE = process.env["GITLAB_CLI_FORCE_INSTALL"];
...
const REQUIRE_SIGNATURE = process.env["GITLAB_CLI_REQUIRE_SIGNATURE"] === "1";
...
if (SKIP) {
  console.log(`Skipping ${NAME} binary install (GITLAB_CLI_SKIP_INSTALL / SKIP_INSTALL set).`);
  process.exit(0);
}

if (fs.existsSync(dest) && !FORCE) {
  console.log(`${NAME} binary already present; skipping download (set GITLAB_CLI_FORCE_INSTALL=1 to redownload).`);
  process.exit(0);
}
